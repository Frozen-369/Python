while True:
    guess = int(input("Enter the randon number between 1 to 100 you want player to guess : "))
    user = None
    guess_count = 0
    while guess != user:
    ...
    try:
        with open("PG_Highscore_1.txt", "r") as PG_game:
            high_score = int(PG_game.read())
    except FileNotFoundError:
        with open("PG_Highscore_1.txt", "w") as PG_game:
